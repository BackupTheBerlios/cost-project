...
setGeneric("AL.multi", function(object,
                                ...
                                grps=NULL, #ex: "tempStrata","spaceStrata","techStrata"
                                age.plus=-1,
                                ...){
...
setMethod("AL.multi", signature(object="csData"), function(object,
                                                           tempStrata=NULL, #ex: "year","quarter","month"
                                                           spaceStrata=NULL, #ex: "area","rect"
                                                           techStrata=NULL, #ex: "commCat"
                                                           elmts=list(tp="all",sp="all",tc="all"),
...
eval(parse('',text=paste("Mm <- multinom(Age~Length",c(paste("*",paste(c(tempStrata,spaceStrata,techStrata),collapse="*"),"-1",sep=""),"")[c(!ntest,ntest)],
...
setMethod("plot",signature(x="ALmult"), function(x, #'ALmult' object
                                                 y=NULL,
...
if (is.null(dots$xlab)) dots$xlab <- "Length" ; if (is.null(dots$ylab)) dots$ylab <- "Age Proportion at Length" 
if (is.null(dots$main)) dots$main <- "Age-at-length multinomial analysis"
...
Ls <- length(stratas) ; if (Ls==0) show.legend <- ""
...
setGeneric("plot.ALmult", function(x, 
                                   show.legend="right", 
                                   ...){
	standardGeneric("plot.ALmult")}
)



setMethod("plot.ALmult",signature(x="ALmult"), function(x, 
                                                        show.legend="right",
                                                        ...){
