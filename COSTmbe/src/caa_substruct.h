...
typedef struct
{
  ...
  */
  double   **tau;
  /*! \brief Precisions for observation
    corresponds to precision of haul effect in age model
   */
  double  ***prior_mean;  // Prior mean of fixed effects
  double  ***prior_prec;  // Parameters for gamma-prior on precisions
  double    *prior_prec_obs;  // Parameters for gamma-prior on obs(haul for age)-precisions
  double   **prior_ar;  // Parameters for beta-prior on ar-coeff
  double     tau_obs;
  /*! \brief Loglikelihood */
  double     loglik;
  /*! \brief Number of variables to be simulated */
  int        num_var;
  /*! \brief MCMC samples from all iterations

    See the write_it routine for format of this vector
   */
  double    *mcmc;
} Eff_str;
...
typedef struct
{
  ...
  double   **Q;              /*!< \brief Q_matrix for age parameters */
  double   **Q_new;          /*!< \brief Q_matrix for age parameters */
  ...
  GMRFLib_constr_tp *constr; /*!< \brief Constraints for age model */
