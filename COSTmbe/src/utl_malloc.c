/* $Id: utl_malloc.c,v 1.1 2009/06/04 12:49:11 davidhirst Exp $ */
/*F:utl_malloc*
...
Bugs: Compilers and lint might complain about mixing of data-types. Just
      ignore them.
Author:		gutorm hogasen and Frode Georgsen, NR
See Also: malloc(3), free(3), calloc(3)
Libraries: Include files and libraries found on /local/bild/
Date: Last update 11/12/91
Cvs module: utl_malloc
File: $Id: utl_malloc.c,v 1.1 2009/06/04 12:49:11 davidhirst Exp $
________________________________________________________________
		   ...
		   int i_clear)
{
  ...
  if (i_clear)
    a =  calloc( (unsigned)elements, (unsigned)i_element_size );
  else
    a =  malloc( (unsigned) elements * i_element_size );
  if (!a)
    {
		   ...
		   int i_clear)
{
  ...
  /* Allocate the matrix */
  if (i_clear)
    *m = calloc( (unsigned) rows * cols, (unsigned) i_element_size );
  else
    *m =  malloc( (unsigned)rows * cols * i_element_size );
  if (!*m)
    {
...
/* Allocate the cube: */
  if (i_clear)
    m[i_x_min][i_y_min] = calloc((unsigned) xs*ys*zs, (unsigned) i_element_size);
  else
    m[i_x_min][i_y_min] = malloc((unsigned) xs*ys*zs*i_element_size);
...
/* Allocate pointers to lines in the t-direction. This is done by making a
  ...
  m = Mmatrix_3d(i_x_min,i_x_max+1,i_y_min,i_y_max+1,i_z_min,i_z_max+1,
		 sizeof(void *),0);
...
/* Allocate the 4d-box: */
  if (i_clear)
    m[i_x_min][i_y_min][i_z_min] = 
      calloc((unsigned) xs*ys*zs*ts, (unsigned) i_element_size);
  else
    m[i_x_min][i_y_min][i_z_min]=malloc((unsigned) xs*ys*zs*ts*i_element_size);
...
/* Allocate pointers to lines in the v-direction. This is done by making a
  ...
  m = Mmatrix_4d(i_x_min,i_x_max+1,i_y_min,i_y_max+1,i_z_min,i_z_max+1,
...
/* Allocate the 5d-box: */
  if (i_clear)
    m[i_x_min][i_y_min][i_z_min][i_t_min] = 
